...
namespace Jogo {
    namespace Menu {
        MenuJogador::MenuJogador(Gerenciador::GerenciadorGrafico& gg) : Menu{ gg }, imprimiu{ false },
            ...
            gb.adicionarBotao(new Botao({ 100.0f, 260.0f }, { 100, 40 }, "Menu Principal", [this] {setCodigoRetorno(irMenuPrincipal); }));
        }
        int MenuJogador::executar()
        {
            int ret = Menu::executar();
            gb.adicionarBotao(new Botao({ 200.0f, 40.0f }, { 50 }, "NÚMERO DE JOGADORES: "));
            gb.adicionarBotao(new Botao({ 300.0f, 260.f }, { 100, 40 }, "Jogar", [this] {setCodigoRetorno(comecarPrimeiraFase); }));
        ...
        const bool MenuJogador::usarDoisJogadores(int codigoRetorno) const {
